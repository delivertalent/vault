$(document).ready(function() {
    ...
    $( "#customer_first_name" ).autocomplete({
      //source: 'get_customerList?country='+$('#job_client_id').val(''),
      source: function(request, response) {
        var wordid = $('#job_client_id').val();
        $.ajax({
          url: "../admin-get-customerList",
               dataType: "json",
          data: {
            term : request.term,
            country : wordid
                                ...
                                success : function(data) {
                                    ...
                                    if(data.status == "success"){
                                        ...
                                        $('#sample_2').dataTable().fnAddData( [
                                            '<a href="javascript:void(0)"><b>'+data.isertedID+'</b></a>',
                                            $('#job_name').val(),
                                            data.customerName,
                                            data.designerName,
                                            status,
                                    ...
                                    if(data.status == "updated"){
                                        ...
                                        var status ='';
                                         ...
                                         oTable.fnUpdate( $('#job_name').val(), parseInt(rowPos), 1);  
                                         oTable.fnUpdate( data.customerName, parseInt(rowPos), 2); 
                                         oTable.fnUpdate( data.designerName, parseInt(rowPos), 3);   
                                         oTable.fnUpdate( status, parseInt(rowPos), 4);   
...
$('body').on('click','.del',function(){    
    var r=confirm("Do you want to Delete this Job? All inventories of this job will be also Removed.");
    if (r==true){
    ...
    return {
        ...
        init: function () {
           ...
           $('#sample_work').dataTable({
            ...
            jQuery('#sample_work_wrapper .dataTables_filter input').addClass("form-control input-small"); // modify table search input
            jQuery('#sample_work_wrapper .dataTables_length select').addClass("form-control input-xsmall"); // modify table per page dropdown
            jQuery('#sample_work_wrapper .dataTables_length select').select2(); // initialize select2 dropdown
