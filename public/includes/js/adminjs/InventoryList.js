                               ...
                               //inv_delivery_status inv_warehouse inv_bin inv_binltr inv_note
                                ...
                                success : function(data) {
                                    ...
                                    if(data.status == "success"){
                                        ...
                                        if($('#inv_delivery_status').val() ==1)
                                            Dstatus = "Not yet received";
                                        else if($('#inv_delivery_status').val() ==2)
                                            Dstatus = "In storage";
                                        else if($('#inv_delivery_status').val() ==3)
                                            Dstatus = "Pulled/loaded";
                                        else if($('#inv_delivery_status').val() ==4)
                                            Dstatus = "Delivered";
                                        else{
                                            Dstatus = "";
                                        }
...
$('body').on('click','.editTD',function(){        
    var invId = this.id;
    var curValue = $("#"+invId).text();

    var splitID = invId.split("_");
    var previousID = $("#editingID").val();
    if(previousID!=''){
        var splitPrevID = previousID.split("_");
        if(splitPrevID[1]==2 || splitPrevID[1]==3 || splitPrevID[1]==4 || splitPrevID[1]==6 || splitPrevID[1]==7){
          $("#"+previousID).html($("#editInline option:selected").text());    
        }
        else
            $("#"+previousID).html($("#editInline").val());

        $("#"+previousID).addClass("editTD"); 
        $("#"+previousID).removeClass("currentTD"); 

        var updInventoryID = splitPrevID[2];
        var updElement = splitPrevID[1];
        var updValue = $("#editInline").val();

            $.ajax({
                type : 'post',
                url : 'inventory-inline-update',
                data : {
                    id : updInventoryID,
                    col1 : updElement,
                    val : updValue,
    ...
    if(splitID[1]==1 || splitID[1]==5 || splitID[1]==9 ){
      $("#"+invId).html('<input type="text" class="putBox" id="editInline" name="editInline" value="'+curValue+'"/>');  
    }
    else if(splitID[1]==8){
            curValue = curValue.replace('<br/>',""); //Remove Html Tags.
            curValue = curValue.replace('<br>',""); //Remove Html Tags.
        $("#"+invId).html('<textarea class="putBox" id="editInline" name="editInline">'+curValue+'</textarea>');
    }
    else if(splitID[1]==2){
            $.ajax({
                type : 'get',
                url : 'itemdescription-inline',
                success : function(data) {
                    $("#"+invId).html(data);
                }
            });//End of Ajax.
        ...
        $("#"+invId).html('<select id="editInline" name="editInline"><option value="1">Received in good condition</option><option value="2">Damaged</option><option value="3">Being repaired – in house</option><option value="4">Being repaired – out for repair</option><option value="5">Awaiting call tag / awaiting pickup</option><option value="6">Picked up</option></select>');
    }




    $("#"+invId).removeClass("editTD"); 
    $("#"+invId).addClass("currentTD"); 
    $("#editInline").focus();
});

$('body').on('blur','td.currentTD',function(){ 
    var previousID = $("#editingID").val();
    if(previousID!=''){
        var updValue = $("#editInline").val();
        var splitID = previousID.split("_");
        if(splitID[1]==2 || splitID[1]==3 || splitID[1]==4 || splitID[1]==6 || splitID[1]==7){
          $("#"+previousID).html($("#editInline option:selected").text());    
        }
        else
            $("#"+previousID).html($("#editInline").val());
        ...
        var updInventoryID = splitID[2];
        var updElement = splitID[1];
