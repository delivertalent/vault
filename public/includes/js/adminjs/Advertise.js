$(document).ready(function() {
	...
	$('#btnAddUser').click(function()
	{
        ...
        $('.alert-success').hide(); 
        $('.alert-danger').hide(); 
        $('#modatTitle').html('Add Advertise Image');
                ...
                submitHandler: function (form) {
                    ...
                    App.blockUI(el);
                          $.ajax({
                                ...
                                data : {
                                    id : $('#did').val(),
                                    advertise_image : $('#advertise_image').val()
                                },
                                dataType : 'json',
                                success : function(data) {
                                    ...
                                    if(data.status == "success"){
                                        ...
                                        var addImg='<img width="530" height="70" src="../without-flash-uploader/images_ad/thumb/'+data.add_images+'" alt="">';
                                        ...
                                        $('.form-group').removeClass('has-error').removeClass('has-success');
                                        $('#advertise_image').val('');
                                        $('.alert-success').html('New Carriers Successfully Created.');   
                                          success2.show();
                                    ...
                                    if(data.status == "updated"){
                                         ...
                                         var status = '';
                                         oTable.fnUpdate( $('#advertise_image').val(), parseInt(rowPos), 1);   
...
$('body').on('click','.del',function(){    
    var r=confirm("Do you want to Delete this Advertise?");
    if (r==true){
    ...
    if(id!=''){
          $.ajax({
                ...
                success : function(data) {
                    ...
                    $('.form-group').removeClass('has-error').removeClass('has-success');
                    $('#did').val(data.id);
                    $('#advertise_image').val(data.advertise_image);

                    $('#rowPos').val(aPos);
                    $('#modatTitle').html('Edit Carriers');
                    $('#responsive').modal('show');     
                }
            });
