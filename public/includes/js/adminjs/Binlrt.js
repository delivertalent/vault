$(document).ready(function() {
	...
	$('#btnAddUser').click(function()
	{
        ...
        $('#did').val('');
        $('#binlrt_warehouse').val('');
        $('#binlrt_bin').val('');
        $('#binlrt_name').val('');
        $('.alert-success').hide(); 
        $('.alert-danger').hide(); 
        $('#modatTitle').html('Add Bin Lrt');
...
$("body").on('change', '#binlrt_warehouse', function() { 
  var warehouseID =  $('#binlrt_warehouse').val();
  if(warehouseID!=''){
        $.ajax({
            type : 'get',
                ...
                submitHandler: function (form) {
                    ...
                    App.blockUI(el);
                          $.ajax({
                                ...
                                data : {
                                    id : $('#did').val(),
                                    binlrt_warehouse : $('#binlrt_warehouse').val(),
                                    binlrt_bin : $('#binlrt_bin').val(),
                                    binlrt_name : $('#binlrt_name').val()
                                },
                                dataType : 'json',
                                success : function(data) {
                                    App.unblockUI(el);
                                    ...
                                    if(data.status == "success"){
                                        ...
                                        $('#sample_2').dataTable().fnAddData( [
                                            ...
                                            $('#binlrt_name').val(),
                                            data.bins.bin_name,
                                            data.warehouses.warehouse_name,
                                            bb
                                        ] );
                                        ...
                                        $('#binlrt_bin').val('');
                                        $('#binlrt_name').val('');
                                        $('.alert-success').html('New Bin Successfully Created.');   
                                          success2.show();
                                    ...
                                    if(data.status == "updated"){
                                         ...
                                         var status = '';
                                         oTable.fnUpdate( $('#binlrt_name').val(), parseInt(rowPos), 1);   
                                         oTable.fnUpdate( data.bins.bin_name, parseInt(rowPos), 2);   
                                         oTable.fnUpdate( data.warehouses.warehouse_name, parseInt(rowPos), 3);   
...
$('body').on('click','.del',function(){    
    var r=confirm("Do you want to Delete this Bin?");
    if (r==true){
   ...
   $('.alert-danger').hide(); 
    ...
    if(id!=''){
          $.ajax({
                ...
                dataType : 'json',
                success : function(data) {
                    var BinVal=data.binlrt_bin;
                    
                    if(data.binlrt_warehouse!=''){
                        $.ajax({
                            type : 'get',
                            url : 'get-warehousebin',
                            data : {
                                warehouseID : data.binlrt_warehouse,
                            },
                            success : function(data) {
                                $('#binlrt_bin').html(data);
                                $('#binlrt_bin').val(BinVal);
                            }
                        });
                    }
                    ...
                    $('.form-group').removeClass('has-error').removeClass('has-success');
                    $('#did').val(data.id);
                    $('#binlrt_warehouse').val(data.binlrt_warehouse);
                    $('#binlrt_name').val(data.binlrt_name);

                    $('#rowPos').val(aPos);
                    $('#modatTitle').html('Edit Bin');
                    $('#responsive').modal('show');     
                }
            });
