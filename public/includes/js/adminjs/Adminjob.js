...
$('body').on('change','#job_client_id',function(){
    var clientID =$("#job_client_id").val();
    if(clientID!=''){
        $.ajax({
            type : 'get',
            url : 'get-lead-designer',
            data : {
                clientID : clientID,
            },
            success : function(data) {
                $('#job_designer_id').html(data);
            }
        });        
    }
});
                    ...
                    App.blockUI(el);
                          $.ajax({
                                ...
                                success : function(data) {
                                    ...
                                    if(data.status == "success"){
                                        var status ='';
                                        var bb='<a class="btn btn-xs red btn-removable del" data-id="1" id="del_'+data.isertedID+'" href="javaScript:void(0);"><i class="fa fa-times"></i>Delete</a>';
                                        if($('#job_status').val() ==1)
                                            status = "New";
                                        ...
                                        $('#sample_2').dataTable().fnAddData( [
                                            '<a href="job-inventory/'+data.isertedID+'/'+$('#job_customer_id').val()+'"><b>'+$('#job_name').val()+'</b></a>',
                                            data.customerName,
                                            data.designerName,
                                            status,
                                            bb
