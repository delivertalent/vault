$(document).ready(function() {
	...
	$('#btnAddUser').click(function()
	{
        ...
        $('.alert-success').hide(); 
        $('.alert-danger').hide(); 
        $('#modatTitle').html('Add Manufacturers');
                          ...
                          $.ajax({
                                ...
                                success : function(data) {
                                    ...
                                    if(data.status == "success"){
                                        ...
                                        $('.form-group').removeClass('has-error').removeClass('has-success');
                                        $('#manuf_name').val('');
                                        $('.alert-success').html('New Manufacturers Successfully Created.');   
                                          success2.show();
                                    ...
                                    if(data.status == "updated"){
                                         ...
                                         var status = '';
                                         oTable.fnUpdate( $('#manuf_name').val(), parseInt(rowPos), 1);   
...
$('body').on('click','.del',function(){    
    var r=confirm("Do you want to Delete this Manufacturers?");
    if (r==true){
    ...
    if(id!=''){
          $.ajax({
                ...
                success : function(data) {
                    ...
                    $('.form-group').removeClass('has-error').removeClass('has-success');
                    $('#did').val(data.id);
                    $('#manuf_name').val(data.manuf_name);

                    $('#rowPos').val(aPos);
                    $('#modatTitle').html('Edit Manufacturers');
                    $('#responsive').modal('show');     
                }
            });
