                ...
                submitHandler: function (form) {
                   ...
                   $(':checkbox:checked').each(function(i){
                      invtorylist[i] = $(this).val();
                          ...
                          $.ajax({
                                type : 'POST',
                                url : 'admin-create-workorder',
                                //Parameter List:
                                //job_name, job_status, job_customer_id, job_designer_id, job_install_date, job_address1, job_address2, job_city,
                                //job_state_id, job_zip, job_development_name, job_gated,job_alarm, job_condo,job_stairs , job_elevator, job_house_sqft, job_comments, 
                                data : {
                                    id : $('#did').val(),
                                    wo_firmid : $('#wo_firmid').val(),
                                ...
                                success : function(data) {
                                    ...
                                    if(data.status == "success"){
                                        var status ='New';
                                        var bb='<a class="btn btn-xs blue btn-editable edit" data-id="1" id="edit_'+data.isertedID+'" href="admin-workorder-details/'+data.isertedID+'"><i class="fa fa-pencil"></i> Edit</a>';
                                        $('#sample_2').dataTable().fnAddData( [
                                            '<a href="admin-workorder-details/'+data.isertedID+'"><b>'+data.isertedID+'</b></a>',
                                            data.designer_name,
                                            data.job_name,
...
$('body').on('change','#wo_firmid',function(){

    var clientID =$("#wo_firmid").val();
    if(clientID!=''){
        $.ajax({
            type : 'get',
            url : 'get-jobList',
