$(document).ready(function() {
	...
	$('#btnAddUser').click(function()
	{
        ...
        $('.alert-success').hide(); 
        $('.alert-danger').hide(); 
        $('#modatTitle').html('Add Item Descriptions');
                          ...
                          $.ajax({
                                ...
                                success : function(data) {
                                    ...
                                    if(data.status == "success"){
                                        ...
                                        $('.form-group').removeClass('has-error').removeClass('has-success');
                                        $('#itds_name').val('');
                                        $('.alert-success').html('New Item Descriptions Successfully Created.');   
                                          success2.show();
                                    ...
                                    if(data.status == "updated"){
                                         ...
                                         var status = '';
                                         oTable.fnUpdate( $('#itds_name').val(), parseInt(rowPos), 1);   
...
$('body').on('click','.del',function(){    
    var r=confirm("Do you want to Delete this Item Descriptions?");
    if (r==true){
    ...
    if(id!=''){
          $.ajax({
                ...
                success : function(data) {
                    ...
                    $('.form-group').removeClass('has-error').removeClass('has-success');
                    $('#did').val(data.id);
                    $('#itds_name').val(data.itds_name);

                    $('#rowPos').val(aPos);
                    $('#modatTitle').html('Edit Item Descriptions');
                    $('#responsive').modal('show');     
                }
            });
