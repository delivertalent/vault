$(document).ready(function() {
	...
	$('#btnAddUser').click(function()
	{
        ...
        $('#did').val('');
        $('#warehouse_locations_name').val('');
        $('.alert-success').hide(); 
        $('.alert-danger').hide(); 
        $('#modatTitle').html('Add Warehouse Location');
                          ...
                          $.ajax({
                                ...
                                success : function(data) {
                                    ...
                                    if(data.status == "success"){
                                        ...
                                        $('.form-group').removeClass('has-error').removeClass('has-success');
                                        $('#warehouse_locations_name').val('');
                                        $('.alert-success').html('New Warehouse Location Successfully Created.');   
                                          success2.show();
                                    ...
                                    if(data.status == "updated"){
                                         ...
                                         var status = '';
                                         oTable.fnUpdate( $('#warehouse_locations_name').val(), parseInt(rowPos), 1);   
...
$('body').on('click','.del',function(){    
    var r=confirm("Do you want to Delete this Warehouse Location?");
    if (r==true){
    ...
    if(id!=''){
          $.ajax({
                ...
                success : function(data) {
                    ...
                    $('.form-group').removeClass('has-error').removeClass('has-success');
                    $('#did').val(data.id);
                    $('#warehouse_locations_name').val(data.warehouse_locations_name);

                    $('#rowPos').val(aPos);
                    $('#modatTitle').html('Edit Warehouse Location');
                    $('#responsive').modal('show');     
                }
            });
